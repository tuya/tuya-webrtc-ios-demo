...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _roomText.placeholder = @"Please modify your clientid、secret、deviceDid and so on.";
...
- (instancetype)initWithFrame:(CGRect)frame {
    if (self = [super initWithFrame:frame]) {
        ...
        _startRegularCallButton.backgroundColor = [UIColor colorWithRed:66.0/255.0 green:200.0/255.0 blue:90.0/255.0 alpha:1.0];
        [_startRegularCallButton setTitle:@"Call room" forState:UIControlStateNormal];
        [_startRegularCallButton addTarget:self action:@selector(onStartRegularCall:) forControlEvents:UIControlEventTouchUpInside];
//        [self addSubview:_startRegularCallButton];
        ...
        _startLoopbackCallButton.backgroundColor = [UIColor colorWithRed:0.0 green:122.0/255.0 blue:1.0 alpha:1.0];
        [_startLoopbackCallButton setTitle:@"Connect to IPC" forState:UIControlStateNormal];
        [_startLoopbackCallButton addTarget:self action:@selector(onStartIPCDebugCall:) forControlEvents:UIControlEventTouchUpInside];
...
- (void)layoutSubviews {
    ...
    CGFloat buttonHeight = (CGRectGetMaxY(self.bounds) - CGRectGetMaxY(_roomText.frame) - kCallControlMargin * 4) / 3;
...
- (void)onStartRegularCall:(id)sender {
    // webrtc 自带的demo逻辑
    [_delegate mainView:self didInputRoom:_roomText.roomText isIPCDebug:NO];
}

- (void)onStartIPCDebugCall:(id)sender {
    // webrtc ipc 逻辑
    // 此处需输入授权码。
    NSString *_AuthorizationCode = _roomText.roomText ;
    if ([_AuthorizationCode length] == 0) {
        _AuthorizationCode = @"xxxxx" ;
    }
    [_delegate mainView:self didInputRoom:_AuthorizationCode isIPCDebug:YES];
}
