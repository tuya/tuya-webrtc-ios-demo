...
- (void)viewDidLoad {
    ...
    if ([[[NSProcessInfo processInfo] arguments] containsObject:loopbackLaunchProcessArgument]) {
        [self mainView:nil didInputRoom:@"" isIPCDebug:NO];
    }
}
...
- (void)activeAudioSession {
    NSString *audioMode = AVAudioSessionCategoryPlayAndRecord;
...
- (void)mainView:(ARDMainView *)mainView didInputRoom:(NSString *)room isIPCDebug:(BOOL)isIPCDebug {
//    if (!room.length) {
//        if (isLoopback) {
...
//    }
    ...
    NSRange matchRange = [regex rangeOfFirstMatchInString:trimmedRoom options:0 range:NSMakeRange(0, trimmedRoom.length)];
    ...
    [[ARDVideoCallViewController alloc] initForRoom:trimmedRoom isIPCDebug:isIPCDebug delegate:self];
